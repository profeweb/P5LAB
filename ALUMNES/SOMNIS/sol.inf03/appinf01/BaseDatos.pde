...
// Emails dels usuaris orde ascendent
String[] getEmailsUsuaris() {
  int numFiles = getNumRowsTaula("usuario");
  String[] info = new String[numFiles];
  int nr=0;
  msql.query( "SELECT email FROM usuario ORDER BY nombre ASC" );
  while (msql.next()) {
    info[nr] = msql.getString("email");
    nr++;
  }
  return info;
}
...
// Info de les categories en orde ascendent
String[][] getInfoCategories() {
  int numFiles = getNumRowsTaula("categoria");
  String[][] info = new String[numFiles][2];
  int nr=0;
  msql.query( "SELECT id, nombre FROM categoria ORDER BY nombre ASC" );
  while (msql.next()) {
    info[nr][0] = String.valueOf(msql.getInt("id"));
    info[nr][1] = msql.getString("nombre");
    nr++;
...
void deleteSomni(int id) {
  String q1= "DELETE FROM etiquetasueño WHERE sueño_id='"+id+"'";
  println("DELETE: "+q1);
  msql.query(q1);
  println("DELETE OK! :)");
  String q2= "DELETE FROM sueño WHERE id='"+id+"'";
  println("DELETE: "+q2);
  msql.query(q2);
  println("DELETE OK! :)");
}
