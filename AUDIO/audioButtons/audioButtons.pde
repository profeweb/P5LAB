...
void draw(){
  ...
  ellipse(width/3, height/2, 250, 250);  // Esquerra  ---> so1
  
  
  float d2 = dist(2*width/3, height/2, mouseX, mouseY);
  if(d2<125) {
    fill(0, 255, 0); cursor(HAND);
  }
  else {
    fill(100); cursor(ARROW);
  }
  ellipse(2*width/3, height/2, 250, 250);  // Dreta   ---> so2
}

void mousePressed(){
  float d1 = dist(width/3, height/2, mouseX, mouseY);
  if(d1<125){
    if(so1.isPlaying()){ so1.pause(); }
    else{  so1.play(); }
  }
  
  float d2 = dist(2*width/3, height/2, mouseX, mouseY);
  if(d2<125){
    if(so2.isPlaying()){ so2.pause();  }
    else{  so2.play(); }
  }
}
