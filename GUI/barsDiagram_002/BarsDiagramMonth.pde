...
class BarsDiagramMonth {
  ...
  BarsDiagramMonth(float x, float y, float w, float h){
    this.x = x; this.y = y; this.w = w; this.h = h;
  }
  
  // Setters
  
  void setTexts(int numDays){
    this.texts = new String[numDays];
    for(int i=1; i<=numDays; i++){
      this.texts[i-1] = String.valueOf(i);
    }
  }
  ...
  void display(){
    ...
    for(int i=0; i<this.values.length; i++){
      ...
      float barValue = map(this.values[i], 0, maxValue, 0, h);
      float xBar = this.x + widthBar*i;
      
      fill(colorBars); stroke(0); strokeWeight(5);
