class Ruta {
  ...
  void display() {
    ...
    // Temporada Ruta
    fill(0, 0, 255); textSize(28); textAlign(RIGHT);
    text(this.temporada, x + w, y - 50);
    
    // Tipo Ruta
    fill(255,0,0); textSize(24); textAlign(RIGHT);
  ...
  void setMinMaxLatLon(){
    ...
    for(Lugar l : lugares){
      ...
      if(lat> maximumLat){ maximumLat = lat; }
      if(lat< minimumLat){ minimumLat = lat; }
      if(lon> maximumLon){ maximumLon = lon; }
      if(lon< minimumLon){ minimumLon = lon; }
    }
